...
CREATE OR REPLACE FUNCTION sys.tinyintrowversion(sys.TINYINT, integer, boolean)
RETURNS sys.ROWVERSION
AS 'babelfishpg_common', 'int2rowversion'
LANGUAGE C IMMUTABLE STRICT PARALLEL SAFE;

CREATE CAST (sys.TINYINT AS sys.ROWVERSION)
WITH FUNCTION sys.tinyintrowversion (sys.TINYINT, integer, boolean) AS ASSIGNMENT;
...
CREATE OR REPLACE FUNCTION sys.rowversiontinyint(sys.ROWVERSION)
RETURNS sys.TINYINT
AS $$
  SELECT sys.rowversionint2($1)::sys.TINYINT;
$$
LANGUAGE SQL IMMUTABLE STRICT PARALLEL SAFE;

CREATE CAST (sys.ROWVERSION as sys.TINYINT)
WITH FUNCTION sys.rowversiontinyint (sys.ROWVERSION) AS ASSIGNMENT;
